# ======================================================================================
# BLOCK 1: CÀI ĐẶT, IMPORT VÀ CẤU HÌNH NÂNG CAO (Version Final - Sửa lỗi đường dẫn)
# ======================================================================================
...
try:
    ...
    if response.status_code == 200:
        ...
        print(f"Tai va dang ky font '{PDF_FONT_NAME}' thanh cong.")
    else:
...
class CONFIG:
    ...
    # !!! THAY ĐỔI: Thay thế ID bằng đường dẫn tương đối từ "My Drive" ---
    # Ví dụ: "NCKH/KetQuaHuanLuyen"
    # Script sẽ tự động tạo thư mục này nếu chưa có.
    DRIVE_OUTPUT_PATH = "Tai_Lieu_NCKH/newAiData/cnn"
...
# Gọi hàm set_seed ngay từ đầu
set_seed(CONFIG.SEED)

# --- Setup Môi trường ---
drive.mount(CONFIG.DRIVE_MOUNT_PATH)
# --- THAY ĐỔI: Tạo đường dẫn đầy đủ và chính xác ---
...
# ======================================================================================
# BLOCK 3: DATASET VÀ DATALOADER (Thêm công tắc Data Augmentation)
# ======================================================================================
...
if CONFIG.USE_DATA_AUGMENTATION:
    ...
    print("Data Augmentation da duoc TAT.")
...
class CoughDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if waveform is None: return {"image": torch.zeros(3, CONFIG.IMAGE_SIZE, CONFIG.IMAGE_SIZE), "labels": torch.tensor(-1, dtype=torch.long)}
...
print("Thuc hien chia du lieu theo nhom (Group Split)...")
...
print(f"Da quet {len(df)} files tu {df['participant_id'].nunique()} ca nhan duy nhat.")
...
print(f"\nChia du lieu hoan tat:")
print(f" - Tap Train: {len(train_files)} files tu {len(train_df['participant_id'].unique())} nguoi.")
print(f" - Tap Validation: {len(val_files)} files tu {len(val_df['participant_id'].unique())} nguoi.")
print(f" - Tap Test: {len(test_files)} files tu {len(test_df['participant_id'].unique())} nguoi.")

print("\nDang tinh toan Class Weights de xu ly mat can bang du lieu...")
...
print(f"Class weights da duoc tinh toan: {class_weights.cpu().numpy()}")
for i, name in idx_to_class.items():
    print(f" - Lop '{name}' (id {i}): weight = {class_weights[i]:.2f}")
...
# ======================================================================================
# BLOCK 5: VÒNG LẶP HUẤN LUYỆN (Sử dụng Class Weights)
# ======================================================================================
...
current_time_str = get_vn_time_str(); model.eval()
...
print(f"Da luu cac bieu do hieu suat.")
...
for class_idx, class_name in idx_to_class.items():
    ...
    avg_cam_info = (f"Grad-CAM Trung binh cho lop '{class_name}' (dua tren {len(cam_arrays)} mau)", avg_cam_path); avg_cam_paths.append(avg_cam_info); print(f"   -> Da luu Grad-CAM trung binh tai: {avg_cam_path}")
